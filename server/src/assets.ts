...
function getEventTitle(type: string) {
  ...
  switch (type) {
    ...
    case "list": {
      title = "listed this asset";
      break;
    }
    case "purchase": {
      title = "purchased this asset";
      break;
    }
    case "cancel": {
...
assets.post("/event", async (req: Request, res: Response) => {
  try {
    const { address, tokenId, type, price, fiatPrice, userAddress } = req.body;
    const uuid = `${address.substring(0, 10)}+${tokenId}`;
    const docRef = doc(db, "assets", uuid);
